...
knc.fit(X_train1_balanced, y_train1_balanced)
print("accuracy with " + str(best_ki) + " neighbors: " + str(knc.score(X_validation, y_validation)))
pred = knc.predict(X_validation)
print(sklearn.metrics.confusion_matrix(y_validation, pred))
print(metrics.classification_report(y_validation, pred))
...
for ki in range(1,30,2):
    ...
    print(metrics.classification_report(y_validation, pred))
    # Save ki with the best recall score for class 0
    if sklearn.metrics.recall_score(y_validation, pred, average=None)[0] > best_recall1:
        best_recall1 = sklearn.metrics.recall_score(y_validation, pred, average=None)[0]
        best_ki1 = ki
    lr.append(sklearn.metrics.recall_score(y_validation, pred, average=None)[0])
    lr1.append(sklearn.metrics.recall_score(y_validation, pred, average=None)[1])
...
# Best ki with validation
print("---- BEST RESULTS DOWNSAMPLED ----")
knc = nb.KNeighborsClassifier(n_neighbors=best_ki1)
knc.fit(X_train1_downsampled, y_train1_downsampled)
print("accuracy with " + str(best_ki1) + " neighbors: " + str(knc.score(X_validation, y_validation)))
pred = knc.predict(X_validation)
print(sklearn.metrics.confusion_matrix(y_validation, pred))
print(metrics.classification_report(y_validation, pred))


# Best results are the balanced ones, get results with original test now
print("---- BEST RESULTS GLOBAL ----")
# Balance original train data with the same proportion as train1
Train_Balanced = train_copy.drop(train.query('readmitted == 1').sample(frac=0.87108, random_state=1).index)
